...
class WBFeedbacks:
    def __init__(self, token: str):
        ...
        self.last_retry_after = 0   # сек, которые WB попросил ждать на последнем 429
    ...
    # Сколько раз пробуем; короткий backoff для обрывов/5xx
    MAX_ATTEMPTS = 4
    NET_BACKOFF_CAP = 30
    # Если WB просит ждать дольше этого — НЕ ретраим на месте (иначе штраф растёт),
    # отдаём 429 наверх, и прогон корректно завершится, продолжив в следующий раз.
    BIG_WAIT_SEC = 60

    def _request(self, method: str, path: str, **kwargs):
        """Повторяет при 429/5xx/обрыве связи. Возвращает ответ или None.
        429: Retry-After соблюдаем ПОЛНОСТЬЮ (без обрезки). Если ожидание больше
        BIG_WAIT_SEC — не ждём и не долбим, а возвращаем 429-ответ наверх, чтобы
        вызывающий код аккуратно завершил прогон (отзыв обработается в следующий раз)."""
        url = BASE + path
        last = None
        for attempt in range(self.MAX_ATTEMPTS):
            try:
                ...
                # обрыв связи и т.п. — короткий backoff и повтор, НЕ падаем
                time.sleep(min(5 * (attempt + 1), self.NET_BACKOFF_CAP))
                continue
            last = r
            if r.status_code == 429:
                wait = self._retry_after(r, default=16)
                self.last_retry_after = wait
                if wait > self.BIG_WAIT_SEC:
                    # долгая пауза от WB — не молотим, пусть прогон завершится
                    return r
                time.sleep(wait)   # соблюдаем Retry-After целиком
                continue
